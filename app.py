...
def render_gauge_svg(current, target, title, unit, color=#ff4b4b)
    ...
    svg_code = (
        ...
        f'text x=100 y=95 text-anchor=middle font-size=11 fill=#64748bZiel {target} {unit}text'
        'svg'
        f'div style=margin-top 8px; font-size 13px; font-weight 600; color {color};{pct_int}% erreichtdiv'
        'div'
    )
...
if st.session_state['nav_tab'] == 🏠 Startseite
    ...
    drink_list = []
    if d['wasser_soda']  0 drink_list.append(f{d['wasser_soda']}L WasserSoda)
    ...
    if w['zirkel_min']  0 workout_list.append(fZirkel {w['zirkel_min']}m)
    if w['bike_km']  0 workout_list.append(fBike {w['bike_km']}km)
    ...
    descriptions = []
    if m['fruehstueck']['desc'] descriptions.append(fFrühstück {m['fruehstueck']['desc']})
    if m['mittagessen']['desc'] descriptions.append(fMittag {m['mittagessen']['desc']})
    if m['abendessen']['desc'] descriptions.append(fAbend {m['abendessen']['desc']})
    if m['snacks'] descriptions.append(Snacks vorhanden)
    descriptions.append(fGetränke {drink_summary})
    descriptions.append(fTraining {workout_summary})
